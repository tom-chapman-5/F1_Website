...
function displayDriver(driverName) {
	fetch("https://raw.githubusercontent.com/tom-chapman-5/driver_data/refs/heads/main/Haas_drivers.json")
		.then(response => {
			if (!response.ok) {
		...
		.then(data => {
			...
			if (driver) {
				...
				driverCard.innerHTML = `
					...
					<h3>Stats with Haas</h3>
					<p>Seasons: ${driver.seasons_haas}</p>
					<p>Wins: ${driver.wins_haas}</p>
					<p>Poles: ${driver.poles_haas}</p>
					<p>Races: ${driver.races_haas}</p>
...
function displayStats() {
	...
	driverCard.innerHTML = `
		<h2>Stats</h2>
			...
			<li style="float: none;">Races: 196</li>
		</ul>
		<h2>2025 Stats</h2>
		<ul>
			<li style="float: none;">Race Wins: 0</li>
			...
			<li style="float: none;">Points: 20</li>
			<li style="float: none;">Podiums: 0</li>
		</ul>
